/* Copyright (C) 2016 Benjamin Wulff
 *
...
public class RTSPClientFrameGrabber implements FrameGrabber {
  ...
  public RTSPClientFrameGrabber(String location) throws FrameSourceException {
    this.location = location;
    pipeline = createPipeline(location);
    start();
    getVideoFrameSize();
  }

  private Pipeline createPipeline(String location) throws IllegalStateException {
    Pipeline pipeline = new Pipeline("RTSP Client");

    // TODO - cannot link rtspsrc directly, needs a callback function
    // See http://gstreamer-devel.966125.n4.nabble.com/Problem-linking-rtspsrc-to-any-other-element-td3051725.html
    // and https://opencast.jira.com/secure/attachment/11948/rtp-producer-review.diff

    Element rtspsrc = createElement("rtspsrc", "rtspsrc");
    rtspsrc.set("location", "rtsp://".concat(location));
    addToPipeline(pipeline, rtspsrc);

    Element rtph264depay = createElement("rtph264depay", "rtph264depay");
    addToPipeline(pipeline, rtph264depay);
    linkElements(rtspsrc, rtph264depay);
